 ...
 * Magnet: sPhenix preCDR 2015 Fig. 10.4
 * Barrel: sPhenix preCDR 2015 Fig. 10.4
 * Forward sPHENIX: ??
 * ?EIC detectors: ephenix_v1 CAD drawing (HCAL) + ECAL and PID detectors and GEM from LOI 2014 ("babar_v11?")
 *
 */
void show_detectorGeometry_v2()
{
  gStyle->SetOptStat(0);
...
//  v_etas.push_back(-4.5);
  ...
  Double_t c_hcal_z[c_hcal_n] = {3.1549,
			  3.1549,
			  -3.1549,
			  -3.1549,
			  -1.5420,
...
//				 0.85*1.82998};
  ...
  /* HSide RICH - Begin V1*/
  Double_t h_rich_center_z = 1.0000;
  Double_t h_rich_center_r = 0.4000;

  // inner ring
  Double_t h_rich_c1_r = 1.0;
  Double_t h_rich_c1_phimin = -20 / 180. * TMath::Pi();
  Double_t h_rich_c1_phimax= 33 / 180. * TMath::Pi(); // LOI: 61
  ...
  Double_t h_rich_c2_phimax= 29 / 180. * TMath::Pi(); // LOI: 50
  ...
  Double_t h_aerogel_z[h_aerogel_n] = {3.5000,
				 4.5000,
				 4.5000,
				 3.5000,
				 3.5000};
			  
  Double_t h_aerogel_r[h_aerogel_n] = {0.0500,
				 0.0500,
				 3.3689,
				 2.6202,
				 0.0050};
  ...
  c_ephenix->Print("detectorGeometry_v2_ePhenix.png");
  c_ephenix->Print("detectorGeometry_v2_ePhenix.eps");




  /* Draw canvas sPHENIX*/
  TCanvas *c_sphenix = new TCanvas();
  hframe->Draw();
  ...
  c_sphenix->Print("detectorGeometry_v2_sPhenix.png");
  c_sphenix->Print("detectorGeometry_v2_sPhenix.eps");


  /* Draw canvas Magnet Yoke*/
  TCanvas *c_yoke = new TCanvas();
  hframe->Draw();
  ...
  c_yoke->Print("detectorGeometry_v2_yoke.png");
  c_yoke->Print("detectorGeometry_v2_yoke.eps");
